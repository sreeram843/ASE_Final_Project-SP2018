...
public abstract class AbstractExpr {    // AbstractExpr should try to be immutable. This means it should not be changed after initialization. parent should
	...
	static public enum ABSTRACTEXPRTYPES
	{
		...
		public static ABSTRACTEXPRTYPES getAETypeFromOpt(BaseData.OPERATORTYPES ot)	{
			switch(ot)	{
	...
	public abstract int[] recalcAExprDim(boolean bUnknownAsSingle) throws SMErrProcessor.JSmartMathErrException, ErrProcessor.JFCALCExpErrException;

	public abstract boolean isEqual(AbstractExpr aexpr) throws ErrProcessor.JFCALCExpErrException;
    ...
    public abstract AbstractExpr copySetListOfChildren(LinkedList<AbstractExpr> listChildren)   throws ErrProcessor.JFCALCExpErrException, SMErrProcessor.JSmartMathErrException;
	
	// this function replaces children who equal aeFrom to aeTo and returns the number of children that are replaced.
	public abstract AbstractExpr replaceChildren(LinkedList<PatternManager.PatternExprUnitMap> listFromToMap, boolean bExpr2Pattern, LinkedList<AbstractExpr> listReplacedChildren) throws ErrProcessor.JFCALCExpErrException, SMErrProcessor.JSmartMathErrException;
	 ...
	 */
	public abstract AbstractExpr simplifyAExpr(LinkedList<UnknownVarOperator.UnknownVariable> lUnknownVars,
	...
	public abstract boolean needBracketsWhenToStr(ABSTRACTEXPRTYPES enumAET, int nLeftOrRight) throws ErrProcessor.JFCALCExpErrException;
    ...
    public abstract BaseData.DataClass evaluateAExprQuick(LinkedList<UnknownVarOperator.UnknownVariable> lUnknownVars,
                                                          LinkedList<LinkedList<Variable>> lVarNameSpaces)
            throws InterruptedException, SMErrProcessor.JSmartMathErrException, ErrProcessor.JFCALCExpErrException;
    ...
    public abstract AbstractExpr evaluateAExpr(LinkedList<UnknownVarOperator.UnknownVariable> lUnknownVars,
                                                LinkedList<LinkedList<Variable>> lVarNameSpaces)
            throws InterruptedException, SMErrProcessor.JSmartMathErrException, ErrProcessor.JFCALCExpErrException;
    ...
    public abstract AbstractExpr convertAExprDatum2AExpr()
            throws SMErrProcessor.JSmartMathErrException;

    public abstract int getVarAppearanceCnt(String strVarName);
    
	// this member function does not change this.
	public AbstractExpr simplifyAExprMost(LinkedList<UnknownVarOperator.UnknownVariable> lUnknownVars, LinkedList<LinkedList<Variable>> lVarNameSpaces, SimplifyParams simplifyParams)
			throws InterruptedException, SMErrProcessor.JSmartMathErrException, ErrProcessor.JFCALCExpErrException {
		...
		while (true)	{
            ...
            if (aeAfter instanceof AEConst && ((AEConst)aeAfter).getDataClassRef().getDataType() != BaseData.DATATYPES.DATUM_ABSTRACT_EXPR) {
	...
	// the same position.
	public abstract int compareAExpr(AbstractExpr aexpr) throws SMErrProcessor.JSmartMathErrException, ErrProcessor.JFCALCExpErrException;
	 ...
	 */
	public static int compareTwoAExprs(AbstractExpr aexpr1, AbstractExpr aexpr2) throws SMErrProcessor.JSmartMathErrException, ErrProcessor.JFCALCExpErrException {
		return aexpr1.compareAExpr(aexpr2);
	}

	public static boolean isCompareOpt(BaseData.CalculateOperator co)	{
		BaseData.OPERATORTYPES ot = co.getOperatorType();
		if (ot == BaseData.OPERATORTYPES.OPERATOR_LARGER || ot == BaseData.OPERATORTYPES.OPERATOR_NOSMALLER
				|| ot == BaseData.OPERATORTYPES.OPERATOR_EQ
				|| ot == BaseData.OPERATORTYPES.OPERATOR_NOLARGER || ot == BaseData.OPERATORTYPES.OPERATOR_SMALLER)	{
			return true;
		}
		return false;
	}

	public static boolean isExprsEqual(AbstractExpr[] aexprs1, AbstractExpr[] aexprs2) throws ErrProcessor.JFCALCExpErrException {
    ...
    public static int mustBeNegativeOrPositive(AbstractExpr aeInput1, AbstractExpr aeInput2) throws ErrProcessor.JFCALCExpErrException {
        if (aeInput1.isEqual(aeInput2)) {
            ...
            if (((AEPosNegOpt)aeInput1).mlistOpts.getFirst().getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_ADD
                    || ((AEPosNegOpt)aeInput1).mlistOpts.getFirst().getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_POSSIGN) {
                return 1;   // pos sign
            } else {
                return -1;  // neg sign
            }
            ...
            if (((AEPosNegOpt)aeInput2).mlistOpts.getFirst().getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_ADD
                    || ((AEPosNegOpt)aeInput2).mlistOpts.getFirst().getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_POSSIGN) {
                return 1;   // pos sign
            } else {
                return -1;  // neg sign
            }
            ...
            for (int idx = 0; idx < aeInput1.getListOfChildren().size(); idx ++) {
                ...
                if (nThisPosNeg != 0) {
                    if ((((AEPosNegOpt)aeInput1).mlistOpts.get(idx).getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_ADD
                            || ((AEPosNegOpt)aeInput1).mlistOpts.get(idx).getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_POSSIGN)
                            && (((AEPosNegOpt)aeInput2).mlistOpts.get(idx).getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_SUBTRACT
                            || ((AEPosNegOpt)aeInput2).mlistOpts.get(idx).getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_NEGSIGN)){
                        nThisPosNeg *= -1;
                    } else if ((((AEPosNegOpt)aeInput2).mlistOpts.get(idx).getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_ADD
                            || ((AEPosNegOpt)aeInput2).mlistOpts.get(idx).getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_POSSIGN)
                            && (((AEPosNegOpt)aeInput1).mlistOpts.get(idx).getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_SUBTRACT
                            || ((AEPosNegOpt)aeInput1).mlistOpts.get(idx).getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_NEGSIGN)) {
                        nThisPosNeg *= -1;
                    }
                } else {
                    nPosNeg = 0;
            ...
            BaseData.DataClass datum1 = ((AEConst)aeInput1).getDataClassRef();
            BaseData.DataClass datum2 = ((AEConst)aeInput2).getDataClassRef();
            if (datum1.getDataType() == BaseData.DATATYPES.DATUM_ABSTRACT_EXPR && datum2.getDataType() == BaseData.DATATYPES.DATUM_ABSTRACT_EXPR) {
                return mustBeNegativeOrPositive(datum1.getAExpr(), datum2.getAExpr());
            } else if (datum1.getDataType() != BaseData.DATATYPES.DATUM_ABSTRACT_EXPR && datum2.getDataType() != BaseData.DATATYPES.DATUM_ABSTRACT_EXPR) {
                if (datum1.isEqual(datum2)) {
                    return 1;
                } else {
                    BaseData.DataClass datum1Neg = ExprEvaluator.evaluateOneOperandCell(new BaseData.CalculateOperator(BaseData.OPERATORTYPES.OPERATOR_NEGSIGN, 1, true), datum1);
