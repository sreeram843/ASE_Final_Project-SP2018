...
public class PatternManager {
	...
	public static AbstractExpr replaceExprPattern(AbstractExpr aeExpression, LinkedList<PatternExprUnitMap> listFromToMap, boolean bExpr2Pattern) throws ErrProcessor.JFCALCExpErrException, JSmartMathErrException	{
	...
	public void loadPatterns(int nFilter) throws JSmartMathErrException, ErrProcessor.JFCALCExpErrException, InterruptedException	{
		if ((nFilter & 1) == 1) {
	...
	public AbstractExpr solveVarByPtnSlvVarIdentifier(AbstractExpr aeExpr2Solve,  //aeExpr2Solve will not be changed in this function
							...
							throws ErrProcessor.JFCALCExpErrException, InterruptedException, JSmartMathErrException	{
		...
		do {
			...
			for (int idx = 0; idx < mlistSimpleVar2SlvPatterns.size(); idx ++)  {
				...
				try {
                    BaseData.DataClass datumValueOfUnknown = new BaseData.DataClass();
	...
	public AbstractExpr[] simplifyByPtnSlvVarIdentifier(AbstractExpr[] aeOriginalExprs,   // individual aexpr will not be changed in this function.
												LinkedList<UnknownVariable> listUnknown,
												LinkedList<LinkedList<Variable>> lVarNameSpaces)
							throws InterruptedException, ErrProcessor.JFCALCExpErrException, JSmartMathErrException	{
	...
	public PtnSlvVarMultiRootsIdentifier findPtnSlvVarMultiRootsIdentifierToMatch(AbstractExpr aeOriginalExpr,
												...
												LinkedList<UnknownVariable> listUnknown,
												LinkedList<LinkedList<Variable>> lVarNameSpaces)
							throws ErrProcessor.JFCALCExpErrException, JSmartMathErrException, InterruptedException	{
	...
	public AbstractExpr integInDefByPtn1VarIntegIdentifier(AbstractExpr aeExpr2Integ,   // it has been simplified most. so need not to do it in the function
		...
		if (aeExpr2Integ instanceof AEConst) {  // integrate a constant.
            ...
            BaseData.DataClass datumExpr2Integ = ((AEConst)aeExpr2Integ).getDataClassCopy();
            datumExpr2Integ.changeDataType(BaseData.DATATYPES.DATUM_COMPLEX);
			...
			AEConst aeTwo = new AEConst(new BaseData.DataClass(BaseData.DATATYPES.DATUM_INTEGER, MFPNumeric.TWO));
			AEConst aeHalf = new AEConst(new BaseData.DataClass(BaseData.DATATYPES.DATUM_DOUBLE, MFPNumeric.HALF));
			...
			BaseData.DataClass datumPowerTo = aePowerTo.getDataClassCopy();
			datumPowerTo.changeDataType(BaseData.DATATYPES.DATUM_DOUBLE);	// power to (a) must be a real value, otherwise throw exception.
			if (datumPowerTo.getDataValue().isEqual(MFPNumeric.MINUS_ONE)) {
				...
				BaseData.DataClass datumOne = new BaseData.DataClass(BaseData.DATATYPES.DATUM_INTEGER, MFPNumeric.ONE);
				BaseData.DataClass datumIntegrated = BuiltinProcedures.evaluateAdding(datumPowerTo, datumOne);
				BaseData.DataClass datum1OverInteg = BuiltinProcedures.evaluateDivision(datumOne, datumIntegrated);
				...
				&& ((AEMulDivOpt)aeExpr2Integ).mlistOpts.getLast().getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_DIVIDE   // must be divide
            ...
            BaseData.DataClass datumCoeff = aeCoeff.getDataClassCopy();
			datumCoeff.changeDataType(BaseData.DATATYPES.DATUM_COMPLEX); // a has to be a single value.
			...
			LinkedList<BaseData.CalculateOperator> listOpts = new LinkedList<BaseData.CalculateOperator>();
			listOpts.add(new BaseData.CalculateOperator(((AEMulDivOpt)aeExpr2Integ).mlistOpts.getFirst().getOperatorType(), 2));
			listOpts.add(new BaseData.CalculateOperator(BaseData.OPERATORTYPES.OPERATOR_MULTIPLY, 2));
			AEMulDivOpt aeReturn = new AEMulDivOpt(listChildren, listOpts);
			return aeReturn;
		} else if (aeExpr2Integ instanceof AEMulDivOpt && ((AEMulDivOpt)aeExpr2Integ).mlistChildren.size() == 2
			...
			AEConst aeCoeff = (AEConst)((AEMulDivOpt)aeExpr2Integ).mlistChildren.getFirst();
            BaseData.DataClass datumCoeff = aeCoeff.getDataClassCopy();
            datumCoeff.changeDataType(BaseData.DATATYPES.DATUM_COMPLEX);
			...
			BaseData.DataClass datumPowerTo = aePowerTo.getDataClassCopy();
			datumPowerTo.changeDataType(BaseData.DATATYPES.DATUM_DOUBLE); // b has to be a real single value.
			if (((AEMulDivOpt)aeExpr2Integ).mlistOpts.getLast().getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_DIVIDE) {
			...
			if (datumPowerTo.getDataValue().isEqual(MFPNumeric.MINUS_ONE)) { // a/x
				...
				BaseData.DataClass datumOne = new BaseData.DataClass(BaseData.DATATYPES.DATUM_INTEGER, MFPNumeric.ONE);
				BaseData.DataClass datumIntegrated = BuiltinProcedures.evaluateAdding(datumPowerTo, datumOne);
				if (((AEMulDivOpt)aeExpr2Integ).mlistOpts.getFirst().getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_MULTIPLY) {
			...
			for (AbstractExpr aeChild : ((AEPosNegOpt)aeExpr2Integ).mlistChildren) {
				...
				listOpts.add(new BaseData.CalculateOperator(((AEPosNegOpt)aeExpr2Integ).mlistOpts.get(idxChild).getOperatorType(),
			...
			if (aeExpr2Integ.getListOfChildren().size() == 2)   {   // size cannot be smaller than 2.
				...
				if (((AEMulDivOpt)aeExpr2Integ).mlistOpts.getLast().getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_DIVIDE) {
					...
					AEConst aeCoeff = (AEConst)((AEMulDivOpt)aeExpr2Integ).mlistChildren.getFirst();
					BaseData.DataClass datumCoeff = aeCoeff.getDataClassCopy();
                    datumCoeff.changeDataType(BaseData.DATATYPES.DATUM_COMPLEX); // coefficient can only be a complex.
                    aeCoeff = new AEConst(datumCoeff);
					...
					listOpts = new LinkedList<BaseData.CalculateOperator>();
					listOpts.add(new BaseData.CalculateOperator(((AEMulDivOpt)aeExpr2Integ).mlistOpts.getFirst().getOperatorType(),
                    ...
                    BaseData.DataClass datumCoeff = ((AEConst)((AEMulDivOpt)aeExpr2Integ).mlistChildren.getFirst()).getDataClassCopy();
                    datumCoeff.changeDataType(BaseData.DATATYPES.DATUM_COMPLEX); // coefficient can only be a complex.
				...
				AEConst aeOne = new AEConst(new BaseData.DataClass(BaseData.DATATYPES.DATUM_INTEGER, MFPNumeric.ONE));
			...
			for (Ptn1VarIntegIdentifier pvii : listAllPatterns) {
				for (int idxPattern = 0; idxPattern < pvii.maearrayPatterns.length; idxPattern ++) {
					...
					if (bMatch) {
						...
						BaseData.CalculateOperator coptCoeff = null;
						...
						for (int idx = 0; idx < pvii.maearrayPseudoConsts.length; idx ++)   {
							...
							listPseudoConsts.add(new Variable(strPseudoConstName, new BaseData.DataClass(BaseData.DATATYPES.DATUM_NULL, MFPNumeric.ZERO)));
						}
						...
						if (pvii.mstrarrayPatternConditions[idxPattern].trim().length() > 0) {
							...
							exprEvaluator.evaluateExpression(pvii.mstrarrayPatternConditions[idxPattern], new BaseData.CurPos());
						}
						boolean bMatchPseudo = true;
						for (int idx = 0; idx < pvii.maearrayPseudoConsts.length; idx ++)   {
							String strPseudoConstName = pvii.maearrayPseudoConsts[idx].mstrVariableName;
							BaseData.DataClass datumPatternCond = VariableOperator.lookUpList4Value(strPseudoConstName, listPseudoConsts);
							BaseData.DataClass datumMatched = new BaseData.DataClass(BaseData.DATATYPES.DATUM_NULL, MFPNumeric.ZERO);
							...
							if (!bMatchPseudo) {
								break;
							} else if (datumPatternCond.getDataType() == BaseData.DATATYPES.DATUM_NULL) {
								// in the pattern condition the pseudo constant's value is not set.
								if (datumMatched.getDataType() == BaseData.DATATYPES.DATUM_NULL) {
							...
							} else if (datumMatched.getDataType() != BaseData.DATATYPES.DATUM_NULL && !datumPatternCond.isEqual(datumMatched)) {
								...
								bMatchPseudo = false;
								break;
							} else if (datumMatched.getDataType() == BaseData.DATATYPES.DATUM_NULL) {
						...
						for (int idx = 0; idx < pvii.mstrarrayPCConditions.length; idx ++) {
							...
							BaseData.DataClass datumPCCondition = exprEvaluator.evaluateExpression(pvii.mstrarrayPCConditions[idx], new BaseData.CurPos());
							datumPCCondition.changeDataType(BaseData.DATATYPES.DATUM_BOOLEAN);
						...
						if (aeUnknownExprUnit.menumAEType != ABSTRACTEXPRTYPES.ABSTRACTEXPR_VARIABLE) {
							if (aeUnknownExprUnit instanceof AEMulDivOpt && ((AEMulDivOpt)aeUnknownExprUnit).mlistChildren.size() == 2
									...
									&& ((AEMulDivOpt)aeUnknownExprUnit).mlistOpts.getLast().getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_MULTIPLY) {
								...
								coptCoeff = (((AEMulDivOpt)aeUnknownExprUnit).mlistOpts.getFirst().getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_MULTIPLY)
										?new BaseData.CalculateOperator(BaseData.OPERATORTYPES.OPERATOR_DIVIDE, 2)
										:new BaseData.CalculateOperator(BaseData.OPERATORTYPES.OPERATOR_MULTIPLY, 2);
								...
								coptCoeff = new BaseData.CalculateOperator(BaseData.OPERATORTYPES.OPERATOR_MULTIPLY, 2);
								...
								boolean bNeg = ((AEPosNegOpt)aeUnknownExprUnit).mlistOpts.getFirst().getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_NEGSIGN
										|| ((AEPosNegOpt)aeUnknownExprUnit).mlistOpts.getFirst().getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_SUBTRACT;
								if (bNeg) {
									aeIntegCoeff = new AEConst(new BaseData.DataClass(BaseData.DATATYPES.DATUM_INTEGER, MFPNumeric.MINUS_ONE));
								} else {
									aeIntegCoeff = new AEConst(new BaseData.DataClass(BaseData.DATATYPES.DATUM_INTEGER, MFPNumeric.ONE));
								}
								coptCoeff = new BaseData.CalculateOperator(BaseData.OPERATORTYPES.OPERATOR_MULTIPLY, 2);
							} else {
						...
						AbstractExpr aeIntegrated = ExprAnalyzer.analyseExpression(pvii.mstrIntegrated, new BaseData.CurPos(), listPseudoConsts);
						...
						if (aeIntegCoeff != null)   {
							...
							LinkedList<BaseData.CalculateOperator> listMulDivOpts = new LinkedList<BaseData.CalculateOperator>();
							listMulDivOpts.add(coptCoeff);
							listMulDivOpts.add(new BaseData.CalculateOperator(BaseData.OPERATORTYPES.OPERATOR_MULTIPLY, 2));
			...
			if (aeExpr2Integ instanceof AEMulDivOpt) {
				...
				for (int idx = 0; idx < ((AEMulDivOpt)aeExpr2Integ).mlistChildren.size(); idx ++) {
					...
					if (((AEMulDivOpt)aeExpr2Integ).mlistOpts.get(idx).getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_DIVIDE) {
						...
						LinkedList<BaseData.CalculateOperator> listGrandOpts = new LinkedList<BaseData.CalculateOperator>();
						listGrandOpts.add(new BaseData.CalculateOperator(BaseData.OPERATORTYPES.OPERATOR_MULTIPLY, 2));
						listGrandOpts.add(new BaseData.CalculateOperator(BaseData.OPERATORTYPES.OPERATOR_DIVIDE, 2));
					...
					if (aeChildIntegrated instanceof AEPosNegOpt && aeChildIntegrated.getListOfChildren().size() == 1
							&& (((AEPosNegOpt)aeChildIntegrated).mlistOpts.getFirst().getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_NEGSIGN
								|| ((AEPosNegOpt)aeChildIntegrated).mlistOpts.getFirst().getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_SUBTRACT)) {
						// this is like - cos(x)... don't worry about + cos(x) coz it has been fully simplified.
						aeFactor = new AEConst(new BaseData.DataClass(BaseData.DATATYPES.DATUM_INTEGER, MFPNumeric.MINUS_ONE));
						...
						if (((AEMulDivOpt)aeChildIntegrated).mlistOpts.getFirst().getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_DIVIDE) {
						...
						if (aeChildIntegrated.getListOfChildren().size() == 2) {
							if (((AEMulDivOpt)aeChildIntegrated).mlistOpts.getLast().getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_DIVIDE) {
								LinkedList<AbstractExpr> listGrandChildren = new LinkedList<AbstractExpr>();
								listGrandChildren.add(aeOne);
                    ...
                    LinkedList<BaseData.CalculateOperator> listModifiedCalcOpt = new LinkedList<BaseData.CalculateOperator>();
                    listModifiedCalcOpt.addAll(((AEMulDivOpt)aeExpr2Integ).mlistOpts);
                    listModifiedChildren.set(idx, aeFactor);
					listModifiedCalcOpt.set(idx, new BaseData.CalculateOperator(BaseData.OPERATORTYPES.OPERATOR_MULTIPLY, 2));
	...
	public AbstractExpr integRecurChildren(AbstractExpr aeIntegrated, LinkedList<LinkedList<Variable>> lVarNameSpaces)
		...
		for (int idx = 0; idx < listOfChildren.size(); idx ++) {
			if (listOfChildren.get(idx) instanceof AEFunction
				...
				BaseData.DataClass datumChildReturn = exprEvaluator.evaluateExpression(listOfChildren.get(idx).output(), new BaseData.CurPos());
				AbstractExpr aeChildReturn = null;
				if (datumChildReturn.getDataType() == BaseData.DATATYPES.DATUM_STRING) {
					// seems to return an expression. Note that the returned expression should not include any pseudo const in the parameter.
					aeChildReturn = ExprAnalyzer.analyseExpression(datumChildReturn.getStringValue(), new BaseData.CurPos(), new LinkedList<Variable>());
