...
public class Ptn1VarIntegIdentifier {
    ...
    public Ptn1VarIntegIdentifier(PatternManager.ABSTRACTEXPRPATTERNS enumAEPType, String[] strarrayPatterns, String[] strarrayPatternConditions, String[] strarrayPseudoConsts,
    ...
    public final void setPtn1VarIntegIdentifier(PatternManager.ABSTRACTEXPRPATTERNS enumAEPType, String[] strarrayPatterns, String[] strarrayPatternConditions, String[] strarrayPseudoConsts,
                                                String[] strarrayPCRestricts, String[] strarrayPCConditions, String strVariable, String[] strarraySingularPnts, String strExclIntegRanges,
                                                boolean bIsDefiniteInteg, String strDefIntegEnd1, String strDefIntegEnd2, String strIntegrated)
            throws ErrProcessor.JFCALCExpErrException, SMErrProcessor.JSmartMathErrException, InterruptedException  {
        ...
        LinkedList<UnknownVarOperator.UnknownVariable> listUnknowns = new LinkedList<UnknownVarOperator.UnknownVariable>();
        LinkedList<Variable> listPseudoConsts = new LinkedList<Variable>();
        for (int idx = 0; idx < mstrarrayPseudoConsts.length; idx ++)   {
            maearrayPseudoConsts[idx] = (AEVar) ExprAnalyzer.analyseExpression(mstrarrayPseudoConsts[idx], new BaseData.CurPos());
            UnknownVarOperator.UnknownVariable var = UnknownVarOperator.lookUpList(maearrayPseudoConsts[idx].mstrVariableName, listUnknowns);
            if (var != null)    {
                throw new SMErrProcessor.JSmartMathErrException(SMErrProcessor.ERRORTYPES.ERROR_VARIABLE_REDECLARED);
            }
            listUnknowns.add(new UnknownVarOperator.UnknownVariable(maearrayPseudoConsts[idx].mstrVariableName));
        ...
        for (int idx = 0; idx < maearrayPatterns.length; idx ++) {
            maearrayPatterns[idx] = ExprAnalyzer.analyseExpression(mstrarrayPatterns[idx], new BaseData.CurPos(), listPseudoConsts);
        }
        maeVariable = (AEVar) ExprAnalyzer.analyseExpression(mstrVariable, new BaseData.CurPos()); // it must be a variable, so no need to add listPseudoConsts.
        UnknownVarOperator.UnknownVariable var = UnknownVarOperator.lookUpList(maeVariable.mstrVariableName, listUnknowns);
        if (var != null)    {
            throw new SMErrProcessor.JSmartMathErrException(SMErrProcessor.ERRORTYPES.ERROR_VARIABLE_REDECLARED);
        }
        listUnknowns.add(new UnknownVarOperator.UnknownVariable(maeVariable.mstrVariableName));
