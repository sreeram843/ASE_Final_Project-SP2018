...
public class PtnSlvMultiVarsIdentifier {
	...
	public PtnSlvMultiVarsIdentifier(PatternManager.ABSTRACTEXPRPATTERNS enumAEPType,
	...
	public void setPtnSlvMultiVarsIdentifier(PatternManager.ABSTRACTEXPRPATTERNS enumAEPType,
					PatternSetByString[] pssArray,
					String[] strarrayVars,
					int[] narrayVarOrders,
					String[] strarraySolveVarExprs,
		...
		for (int idx = 0; idx < mpssArray.length; idx ++)	{
			for (int idx1 = 0; idx1 < mpssArray[idx].mstrarrayPseudoConsts.length; idx1 ++)	{
				AbstractExpr aexprTmp = ExprAnalyzer.analyseExpression(mpssArray[idx].mstrarrayPseudoConsts[idx1], new BaseData.CurPos());
				...
				mpsaArray[idx].maePConExprs[idx1] = ExprAnalyzer.analyseExpression(mpssArray[idx].mstrarrayPConExprs[idx1], new BaseData.CurPos());
			}
		}
		for (int idx = 0; idx < mstrarrayVars.length; idx ++)	{
			AbstractExpr aexprTmp = ExprAnalyzer.analyseExpression(mstrarrayVars[idx], new BaseData.CurPos());
			if (!(aexprTmp instanceof AEVar))	{
				throw new JSmartMathErrException(ERRORTYPES.ERROR_INVALID_ABSTRACTEXPRPATTERN);
			}
			...
			marrayaeSolveVarExprs[idx] = ExprAnalyzer.analyseExpression(mstrarraySolveVarExprs[idx], new BaseData.CurPos());
		}
		for (int idx = 0; idx < mpssArray.length; idx ++)	{
			mpsaArray[idx].maePattern = ExprAnalyzer.analyseExpression(mpssArray[idx].mstrPattern, new BaseData.CurPos(), listPseudoConsts);
		}
	}
	 ...
	 */
	public static void lookupToSolveVariables(AbstractExpr aexpr, LinkedList<AbstractExpr> listAEVars, LinkedList<AbstractExpr> listAERootVars) throws ErrProcessor.JFCALCExpErrException {
	...
	public static void lookupToSolveVarsInExprs(AbstractExpr[] aexprs, LinkedList<AbstractExpr> listAEVars, LinkedList<AbstractExpr> listAERootVars) throws ErrProcessor.JFCALCExpErrException, JSmartMathErrException	{
		if (listAERootVars == null)	{
			listAERootVars = new LinkedList<AbstractExpr>();
		}
	...
	public BaseData.DataClass[] solveOriginalExprUnit(PatternManager.PatternExprUnitMap peuMap, LinkedList<UnknownVariable> listPseudoConstVars)
		...
		for (int idx1 = 0; idx1 < marrayaeVars.length; idx1 ++)	{
			if (marrayaeVars[idx1].isEqual(aePatternUnit))	{
				...
				if (mbarrayRootsList[idx1] && ((AEConst)aeReturn).getDataClassRef().getDataType() == BaseData.DATATYPES.DATUM_REF_DATA)	{
	...
	public LinkedList<BaseData.DataClass[]> solveOriginalExprUnits(LinkedList<PatternManager.PatternExprUnitMap> listPEUMap,
                                                                   LinkedList<UnknownVariable> listPseudoConstVars) throws ErrProcessor.JFCALCExpErrException, InterruptedException, JSmartMathErrException	{
		LinkedList<BaseData.DataClass[]> listReturnValues = new LinkedList<BaseData.DataClass[]>();
		
		for (int idx = 0; idx < listPEUMap.size(); idx ++)	{
			BaseData.DataClass[] datumReturnList = solveOriginalExprUnit(listPEUMap.get(idx), listPseudoConstVars);
