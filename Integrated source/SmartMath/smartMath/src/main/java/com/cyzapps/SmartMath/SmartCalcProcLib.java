...
public class SmartCalcProcLib {
	...
	public static String calculate(Context context, String strExpressions, boolean bShowInValidAExpr)
	{
		...
		FuncEvaluator.msfunctionInterrupter = new ActivitySmartCalc.SmartCalcFunctionInterrupter();
		...
		ScriptAnalyzer.msscriptInterrupter = new ActivitySmartCalc.SmartCalcScriptInterrupter();
		AbstractExpr.msaexprInterrupter = new ActivitySmartCalc.SmartCalcAbstractExprInterrupter();
        ...
        LinkedList<UnknownVarOperator.UnknownVariable> listVarUnknown = new LinkedList<UnknownVarOperator.UnknownVariable>();	// The unknown variable list
			...
			if (strarrayExprs[idx].trim().length() == 0)	{
				...
				try	{
                    ...
                    if (aeSimplified instanceof AEConst)	{ // ok, we get the value!
                    	...
                    	if (aexpr instanceof AEFunction) {
                    		...
                    		if (strFuncName.equalsIgnoreCase("integrate") && ((AEFunction)aexpr).mlistChildren.size() == 2
                    				&& ((AEConst) aeSimplified).getDataClassRef().getDataType() == BaseData.DATATYPES.DATUM_STRING) {
                    	...
                    	if (bConvertOutput2Expr) {
                    		try {
                    			...
                    			AbstractExpr aeIndefIntegResult = ExprAnalyzer.analyseExpression(((AEConst) aeSimplified).getDataClassRef().getStringValue(), new BaseData.CurPos());
					...
					if (bShowInValidAExpr) {	// camera calculation preview will not show any invalid expression.
						...
						if (e instanceof SMErrProcessor.JSmartMathErrException
								&& ((SMErrProcessor.JSmartMathErrException)e).m_se.m_enumErrorType
		...
		if (listaeInputExprs.size() != 0)	{
			...
			for (int idx = 0; idx <listaeInputExprs.size(); idx ++)	{
				try	{
		            ...
		            if ( aeReturn instanceof AECompare && ((AECompare) aeReturn).moptType == BaseData.OPERATORTYPES.OPERATOR_EQ)	{
			...
			if (bContinue2Solve)	{
				try	{
	                ...
	                LinkedList<UnknownVarOperator.UnknownVariable> listAlreadyPrinted = UnknownVarOperator.cloneUnknownVarList(listVarUnknown);
		            ...
		            if (aeOriginalExprs.length != 0)    {
		            	...
		            	for (int idx = 0; idx < aeOriginalExprs.length; idx ++)	{
		            		AECompare aeEqualZero = new AECompare(aeOriginalExprs[idx],
		            											BaseData.OPERATORTYPES.OPERATOR_EQ,
		            											new AEConst(new BaseData.DataClass(BaseData.DATATYPES.DATUM_DOUBLE, MFPNumeric.ZERO)));
	                    ...
	                    LinkedList<LinkedList<UnknownVarOperator.UnknownVariable>> listAllResultSets = SolveAnalyzer.solveExprVars(SolveAnalyzer.mspm, aeOriginalExprs, listVarUnknown, lVarNameSpaces);
		                if (listAllResultSets.size() > 0)   {
		                	...
		                	for (int idx = 0; idx < listAllResultSets.get(0).size(); idx ++)  {
		                		...
		                		UnknownVarOperator.UnknownVariable varSolved = UnknownVarOperator.lookUpList(strVarName, listAlreadyPrinted);
		                        ...
		                        BaseData.DataClass[] datumList = new BaseData.DataClass[listAllResultSets.size()];
		                        int  nAssignedValue = 0;
		                        for (int idx1 = 0; idx1 < listAllResultSets.size(); idx1 ++)    {
		                            datumList[idx1] = new BaseData.DataClass();
	...
	public static String plot(Context context, Context contextPlot, String strExpressions, long lPlotTriggerTime, boolean bStopWhenInValidAExpr)	{
		...
		LinkedList<LinkedList<UnknownVarOperator.UnknownVariable>> listlVarUnknownExprsAll = new LinkedList<LinkedList<UnknownVarOperator.UnknownVariable>>();
        LinkedList<UnknownVarOperator.UnknownVariable> listVarUnknown = new LinkedList<UnknownVarOperator.UnknownVariable>();	// The unknown variable list
		...
		for (int idx = 0; idx < strarrayExprs.length; idx ++)	{
			...
			if (strarrayExprs[idx].trim().length() == 0)	{
                ...
                try {
					...
					try {
						LinkedList<AbstractExpr> listAEVars = new LinkedList<AbstractExpr>();
				        LinkedList<UnknownVarOperator.UnknownVariable> listVarUnknownSingleExpr = new LinkedList<UnknownVarOperator.UnknownVariable>();
				        LinkedList<AbstractExpr> listAERootVars = new LinkedList<AbstractExpr>();
				        AbstractExpr[] arrayAEs = new AbstractExpr[1];
				        arrayAEs[0] = aexpr;
				        PtnSlvMultiVarsIdentifier.lookupToSolveVarsInExprs(arrayAEs, listAEVars, listAERootVars);					
				        ...
				        if (aexpr instanceof AEAssign)	{
							...
							if (((AECompare) aexpr).moptType == BaseData.OPERATORTYPES.OPERATOR_EQ)	{
		...
		if (bCanPlot == false)	{
			...
			if (listVarUnknown.size() == 3) {
				...
				if (bAllOmitted2Vars)	{	// all the expressions are like x*y, y+z, etc.
					bPlot3DChart = true;
					listVarUnknown.add(new UnknownVarOperator.UnknownVariable(listVarUnknown.get(0).getName() + "_" + listVarUnknown.get(1).getName()));
				} //else some of the exprs are like sin(x) == y, bPlot3DChart is false
			} else if (listVarUnknown.size() == 1)	{
				listVarUnknown.add(new UnknownVarOperator.UnknownVariable("f_" + listVarUnknown.get(0).getName()));
			}
			if (bPlot3DChart)	{
				// three D chart
				...
				FuncEvaluator.msfileOperator = new MFPFileManagerActivity.MFPFileOperator();
				FuncEvaluator.msgraphPlotter = null;
				FuncEvaluator.msgraphPlotter3D = new ActivitySmartCalc.PlotGraphPlotter(contextPlot, lPlotTriggerTime);
                if (FuncEvaluator.mspm == null) {
				...
				try	{
					...
					if (((ActivitySmartCalc.PlotGraphPlotter)(FuncEvaluator.msgraphPlotter3D)).mbOK)	{
				...
				if (listVarUnknown.get(0).getName().trim().equals("\u03B1")	// alpha
					...
					UnknownVarOperator.UnknownVariable varTemp = listVarUnknown.get(1);
				...
				FuncEvaluator.msstreamLogOutput = null;
				FuncEvaluator.msfileOperator = new MFPFileManagerActivity.MFPFileOperator();
				FuncEvaluator.msgraphPlotter = new ActivitySmartCalc.PlotGraphPlotter(contextPlot, lPlotTriggerTime);
				FuncEvaluator.msgraphPlotter3D = null;
                if (FuncEvaluator.mspm == null) {
                    FuncEvaluator.mspm = new PatternManager();	// even if plot we still need mspm because we may calculate integrate.
                    try {
				...
				try	{
					...
					String strErrMsg = MFPAdapter.outputException(e);
					if (((ActivitySmartCalc.PlotGraphPlotter)(FuncEvaluator.msgraphPlotter)).mbOK)	{
						// graph is plotted without problem, but we cannot save the file.
						// cannot use Toast here seems because we are in main thread
						strOutput += "<p>" + context.getString(R.string.graph_is_plotted_but_graph_file_cannot_be_saved) + "</p>";
					} else	{
