...
public class ScriptAnalyzer{
	...
	public class FuncRetException extends ScriptStatementException	{
		...
		public FuncRetException(Statement sFuncRet, BaseData.DataClass datumReturn)
				throws ErrProcessor.JFCALCExpErrException {
			super(sFuncRet);
			m_datumReturn = new BaseData.DataClass();
	...
	public int analyzeBlock(Statement[] sarrayLines,	// statements of this script
			...
			UntilException, ErrProcessor.JFCALCExpErrException,	// until statement throws until exception
		...
		if (sDeclaration.m_statementType.m_strType.equals("function"))	{
			...
			if ((bOptParams == false && sf.m_strParams.length != lParams.size())
				...
				BaseData.DataClass datumOptArgC = new BaseData.DataClass();
				int nOptArgC = lParams.getLast().getValue().getDataListSize();
				datumOptArgC.setDataValue(new MFPNumeric(nOptArgC), BaseData.DATATYPES.DATUM_INTEGER);
		...
		for (index = nDeclarationPosition + 1; index < sarrayLines.length; index ++)	{
			...
			if (sLine.m_statementType.m_strType.equals("function"))	{
				...
				if (sr.m_strReturnedExpr.length() > 0)	{
					...
					BaseData.CurPos c = new BaseData.CurPos();
					c.m_nPos = 0;
					BaseData.DataClass datumReturn;
					try	{
				...
				for (int index1 = 0; index1 < sv.m_strVariables.length; index1 ++)	{
					...
					Variable var = new Variable(sv.m_strVariables[index1], new BaseData.DataClass());
					listVar.add(var);
				}
					...
					if (sv.m_strVarValues[index1].length() != 0)	{
						...
						try	{
							...
							ExprEvaluator.evaluateTwoOperandCell(listVar.get(index1).getValue(),
																new BaseData.CalculateOperator(BaseData.OPERATORTYPES.OPERATOR_ASSIGN, 2),
																datumReturn);
						} catch(ErrProcessor.JFCALCExpErrException e)	{
							ERRORTYPES errType = ERRORTYPES.INVALID_EXPRESSION;
							throw new JMFPCompErrException(sLine.m_nStartLineNo, sLine.m_nEndLineNo, errType, e);												
				...
				BaseData.CurPos c = new BaseData.CurPos();
				c.m_nPos = 0;
				BaseData.DataClass datumCondition;
				try	{
					...
					datumCondition.changeDataType(BaseData.DATATYPES.DATUM_BOOLEAN);
				} catch(ErrProcessor.JFCALCExpErrException e)	{
					ERRORTYPES errType = ERRORTYPES.INVALID_EXPRESSION;
					throw new JMFPCompErrException(sLine.m_nStartLineNo, sLine.m_nEndLineNo, errType, e);												
				}
				String[] strCandidates;
				if (datumCondition.getDataType() == BaseData.DATATYPES.DATUM_BOOLEAN && datumCondition.getDataValue().isActuallyTrue())	{
					/*
					 * condition is true
					 */
				...
				BaseData.CurPos c = new BaseData.CurPos();
				c.m_nPos = 0;
				BaseData.DataClass datumIndex;
				try	{
					BaseData.DataClass datumStart = exprEvaluator.evaluateExpression(sfor.m_strStart, c);
					if (datumStart == null)	{
						ERRORTYPES errType = ERRORTYPES.NO_VALUE_OBTAINED_FROM_EXPRESSION;
						throw new JMFPCompErrException(sLine.m_nStartLineNo, sLine.m_nEndLineNo, errType);
					}
					datumIndex = new BaseData.DataClass();
					exprEvaluator.evaluateTwoOperandCell(datumIndex, new BaseData.CalculateOperator(BaseData.OPERATORTYPES.OPERATOR_ASSIGN, 2), datumStart);
					if (datumIndex.isSingleInteger())	{
						datumIndex.changeDataType(BaseData.DATATYPES.DATUM_INTEGER);
					} else	{
						datumIndex.changeDataType(BaseData.DATATYPES.DATUM_DOUBLE);
					}
				} catch(ErrProcessor.JFCALCExpErrException e)	{
					ERRORTYPES errType = ERRORTYPES.INVALID_EXPRESSION;
					throw new JMFPCompErrException(sLine.m_nStartLineNo, sLine.m_nEndLineNo, errType, e);												
				...
				BaseData.DataClass datumEnd = null;
                int nNumofNameSpaceLevels = 0;  // this variable is used when continue or break exist loop
                                                // coz these two statements may exist from several block levels
				try	{
					...
					while(true)	{
						...
						try	{
							...
							if (datumStep.isSingleInteger())	{
								datumStep.changeDataType(BaseData.DATATYPES.DATUM_INTEGER);
							} else	{
								datumStep.changeDataType(BaseData.DATATYPES.DATUM_DOUBLE);
							}
							if (datumEnd.isSingleInteger())	{
								datumEnd.changeDataType(BaseData.DATATYPES.DATUM_INTEGER);
							} else	{
								datumEnd.changeDataType(BaseData.DATATYPES.DATUM_DOUBLE);
							}
						} catch(ErrProcessor.JFCALCExpErrException e)	{
							ERRORTYPES errType = ERRORTYPES.INVALID_EXPRESSION;
							throw new JMFPCompErrException(sLine.m_nStartLineNo, sLine.m_nEndLineNo, errType, e);												
						...
						if ((datumStep.getDataValue().isActuallyPositive()
							...
							try	{
								...
								} catch(ErrProcessor.JFCALCExpErrException eExpression)	{
									ERRORTYPES errType = ERRORTYPES.INVALID_EXPRESSION;
				...
				if (nsf.m_sThisStatement != null && nsf.m_sThisStatement.m_statementType.m_strType.equals("select"))	{
					...
					BaseData.CurPos c = new BaseData.CurPos();
					c.m_nPos = 0;
					BaseData.DataClass datumSelect;
					try	{
				...
				try {
					datum.changeDataType(BaseData.DATATYPES.DATUM_STRING);
				} catch(ErrProcessor.JFCALCExpErrException e)	{
				...
				if (jmfpeCaught == null)	{
                    ...
                    while (eThisLevel != null)    {
                        ...
                        } else if (eThisLevel instanceof ErrProcessor.JFCALCExpErrException) {
                            ErrProcessor.JFCALCExpErrException jfcalceThisLevel = (ErrProcessor.JFCALCExpErrException)eThisLevel;
					...
					BaseData.DataClass datumExceptionLevel = new BaseData.DataClass();
					datumExceptionLevel.setStringValue(strExceptionLevel);
		
					BaseData.DataClass datumExceptionType = new BaseData.DataClass();
					datumExceptionType.setStringValue(strExceptionType);
					
					BaseData.DataClass datumExceptionInfo = new BaseData.DataClass();
					...
					try {
						...
						datum.changeDataType(BaseData.DATATYPES.DATUM_BOOLEAN);
					} catch(ErrProcessor.JFCALCExpErrException e)	{
				...
				LinkedList<UnknownVarOperator.UnknownVariable> listVarUnknown = new LinkedList<UnknownVarOperator.UnknownVariable>();
				...
				for (int index1 = 0; index1 < ss.m_strVariables.length; index1 ++)	{
					...
					UnknownVarOperator.UnknownVariable varUnknown = new UnknownVarOperator.UnknownVariable(ss.m_strVariables[index1]);
