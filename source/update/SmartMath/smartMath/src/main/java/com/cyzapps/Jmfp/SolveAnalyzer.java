...
public class SolveAnalyzer {
    ...
    public SolveAnalyzer() throws JSmartMathErrException, ErrProcessor.JFCALCExpErrException, InterruptedException    {
        if (mspm == null)   {
	...
	public int analyzeSolve(Statement[] sarrayLines,	// statements of this script
            ...
            if (bInHelpBlock == false)  {
                if (sLine.m_statementType.m_strType.equals("slvreto"))   {
                    ...
                    for (int idx1 = 0; idx1 < listAllResultSets.size(); idx1 ++)    {
                        ...
                        } catch (ErrProcessor.JFCALCExpErrException e)   {   // divided by zero exception etc can be allowed.
                        ...
                        if (!(aexpr instanceof AECompare) || ((AECompare)aexpr).moptType != BaseData.OPERATORTYPES.OPERATOR_EQ)    {
        ...
        BaseData.DataClass datumSlvReTo = new BaseData.DataClass();
        BaseData.DataClass[] arrayDataList = new BaseData.DataClass[listSlvReToResultSets.size()];
        for (int idx = 0; idx < listSlvReToResultSets.size(); idx ++)    {
            BaseData.DataClass datumOneSolvedVars = new BaseData.DataClass();
            BaseData.DataClass[] arraySolvedValues = new BaseData.DataClass[listSlvReToResultSets.get(idx).size()];
            for (int idx1 = 0; idx1 < listSlvReToResultSets.get(idx).size(); idx1 ++)    {
                if (listSlvReToResultSets.get(idx).get(idx1).isValueAssigned())  {
                    ...
                    arraySolvedValues[idx1] = new BaseData.DataClass();  // NULL means value is not solved.
                }
            }
        ...
        String strReturnedVarName = ((Statement.Statement_slvreto)(sSlvReToLine.m_statementType)).m_strReturnedVar;
    ...
    public static BaseData.DataClass[] solveExprVarMultiRoots(PatternManager pm, AbstractExpr aeOriginalExpr,
        ...
        PtnSlvVarMultiRootsIdentifier pa = pm.findPtnSlvVarMultiRootsIdentifierToMatch(aeOriginalExpr,
                                                    ...
                                                    listUnknowns,
                                                    lVarNameSpaces);
        if (pa.menumAEPType == PatternManager.ABSTRACTEXPRPATTERNS.AEP_UNRECOGNIZEDPATTERN
                || listPEUMap.size() != 1)	{
            return new BaseData.DataClass[0];    // cannot recognize.
        }
        BaseData.DataClass[] datumarrayResults = pa.solveOriginalExprUnit(listPEUMap.getFirst(), listPseudoConstVars);
    ...
    public static LinkedList<LinkedList<UnknownVariable>> solveExprVars(PatternManager pm, AbstractExpr[] aeOriginalExprs,
		...
		// apply patterns here
		boolean bIsAllVarsAEVar = true;
		LinkedList<PatternManager.PatternExprUnitMap> listPEUMap = new LinkedList<PatternManager.PatternExprUnitMap>();
        // the list of pseudo const vars in the pattern identifier.
        LinkedList<UnknownVariable> listPseudoConstVars = new LinkedList<UnknownVariable>();
        // need not to deep copy listUnknowns and lVarNameSpaces because if a valid pi is returned,
        // changes of listUnknowns and lVarNameSpaces are desired.
        ...
        LinkedList<BaseData.DataClass[]> listResults = pa.solveOriginalExprUnits(listPEUMap, listPseudoConstVars);
        LinkedList<BaseData.DataClass[]> listAllPossibleResultSets = new LinkedList<BaseData.DataClass[]>();
        ...
        if (!bIsAllVarsAEVar)	{
            ...
            for (int idx = 0; idx < listPEUMap.size(); idx ++)	{
                ...
                for (int idx3 = 0; idx3 < listAllPossibleGroups.size(); idx3 ++)	{
                    LinkedList<BaseData.DataClass> listIdenticalResults = new LinkedList<BaseData.DataClass>();
    ...
    public static LinkedList<AbstractExpr> solveVarInSingleExpr(AbstractExpr aexpr, String strVariable,
            ...
            throws JSmartMathErrException, ErrProcessor.JFCALCExpErrException, InterruptedException, JMFPCompErrException {
        if (!(aexpr instanceof AECompare) || ((AECompare)aexpr).moptType != BaseData.OPERATORTYPES.OPERATOR_EQ || nStackLen > 24) {
        ...
        if (((AECompare)aexpr).maeLeft instanceof AEConst
                && ((AEConst)((AECompare)aexpr).maeLeft).getDataClassRef().getDataType() != BaseData.DATATYPES.DATUM_REF_DATA
            ...
            aexpr = ((AECompare)aexpr).maeRight;
                ...
                && ((AEConst)((AECompare)aexpr).maeRight).getDataClassRef().getDataType() != BaseData.DATATYPES.DATUM_REF_DATA
        ...
        if (!varSolved.isValueAssigned())    {  // unknown variable is not solved
            BaseData.DataClass[] datumarrayResults = new BaseData.DataClass[0];
            try {
                LinkedList<PatternManager.PatternExprUnitMap> listPEUMap = new LinkedList<PatternManager.PatternExprUnitMap>();
                ...
                if (datumarrayResults.length == 0 || listPEUMap.size() != 1) {
                    ...
                    for (int idx2 = 0; idx2 < datumarrayResults.length; idx2 ++) {
                        BaseData.DataClass datum = datumarrayResults[idx2];
                    ...
                    for (int idx = 0; idx < datumarrayResults.length; idx ++) {
                        ...
                        try {
                            ...
                            if (datumarrayResults[idx].getDataType() == BaseData.DATATYPES.DATUM_ABSTRACT_EXPR) {
                                aexprResult = datumarrayResults[idx].getAExpr();
                            } else {
                                aexprResult = new AEConst(datumarrayResults[idx]);
                            }
                            AECompare aeNewExpr = new AECompare(listPEUMap.getFirst().maeExprUnit, BaseData.OPERATORTYPES.OPERATOR_EQ, aexprResult);
            ...
            BaseData.DataClass datum = varSolved.getSolvedValue();
            if (datum.getDataType() == BaseData.DATATYPES.DATUM_ABSTRACT_EXPR) {
                AbstractExpr aexprResult = datum.getAExpr().simplifyAExprMost(listUnknownVars, llistNameSpace, new SimplifyParams(bIgnoreMatrixDim, true, false));
                listResults.add(aexprResult);
            } else {
