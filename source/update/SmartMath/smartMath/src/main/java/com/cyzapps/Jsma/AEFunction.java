...
public class AEFunction extends AbstractExpr {
    ...
    public AEFunction(String strName, LinkedList<AbstractExpr> listChildren) throws SMErrProcessor.JSmartMathErrException {
        setAEFunction(strName, listChildren);
    }

    public AEFunction(AbstractExpr aexprOrigin) throws ErrProcessor.JFCALCExpErrException, SMErrProcessor.JSmartMathErrException {
        copy(aexprOrigin);
    }
    ...
    private void setAEFunction(String strName, LinkedList<AbstractExpr> listChildren) throws SMErrProcessor.JSmartMathErrException {
        menumAEType = ABSTRACTEXPRTYPES.ABSTRACTEXPR_FUNCTION;
	...
	public BaseData.DataClass evaluateAExprQuick(
        ...
        for (int idx = 0; idx < mlistChildren.size(); idx ++)	{
			BaseData.DataClass datumParameter = mlistChildren.get(idx).evaluateAExprQuick(lUnknownVars, lVarNameSpaces);
            listParams.addFirst(datumParameter);    // no need to deep copy. also note that last parameer is the first in the param list.
		}
        BaseData.DataClass datumReturn = FuncEvaluator.evaluateFunction(mstrFuncName, listParams, lVarNameSpaces);
        if (datumReturn == null)    {
    ...
    public AbstractExpr simplifyAExpr(LinkedList<UnknownVarOperator.UnknownVariable> lUnknownVars,
        ...
        for (int idx = 0; idx < aeReturn.mlistChildren.size(); idx ++)    {
            ...
            if (aeChild instanceof AEConst)    {
                ...
                listParams.addFirst(new BaseData.DataClass(aeChild));
                bAllParamSolved = false;
            }
            aeReturn.mlistChildren.set(idx, aeChild);
        }
        if (bAllParamSolved)    {
            BaseData.DataClass datumReturn = FuncEvaluator.evaluateFunction(aeReturn.mstrFuncName, listParams, lVarNameSpaces);
            if (datumReturn == null)    {
                // a function can return nothing, but this function is not supported by smart math.
                throw new SMErrProcessor.JSmartMathErrException(SMErrProcessor.ERRORTYPES.ERROR_FUNCTION_CANNOT_RETURN_NOTHING);
            }
            ...
            AEConst aexprE = new AEConst(new BaseData.DataClass(BaseData.DATATYPES.DATUM_DOUBLE, MFPNumeric.E));
            ...
            AEConst aexprOne = new AEConst(new BaseData.DataClass(BaseData.DATATYPES.DATUM_DOUBLE, MFPNumeric.ONE));
            ...
            AEConst aexprHalf = new AEConst(new BaseData.DataClass(BaseData.DATATYPES.DATUM_DOUBLE, MFPNumeric.HALF));
            ...
            BaseData.CalculateOperator coMultiply = new BaseData.CalculateOperator(BaseData.OPERATORTYPES.OPERATOR_MULTIPLY, 2);
            BaseData.CalculateOperator coDiv = new BaseData.CalculateOperator(BaseData.OPERATORTYPES.OPERATOR_DIVIDE, 2);
