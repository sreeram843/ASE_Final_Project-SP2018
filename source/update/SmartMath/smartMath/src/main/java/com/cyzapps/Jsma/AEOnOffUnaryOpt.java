...
public class AEOnOffUnaryOpt extends AbstractExpr {
	...
	public AEOnOffUnaryOpt(ABSTRACTEXPRTYPES aeType, AbstractExpr aexprChild, int nNumOfOpts) throws SMErrProcessor.JSmartMathErrException {
		setAEOnOffUnaryOpt(aeType, aexprChild, nNumOfOpts);
	}

	public AEOnOffUnaryOpt(BaseData.CalculateOperator calcOpt, AbstractExpr aexprChild, int nNumOfOpts) throws SMErrProcessor.JSmartMathErrException {
		if (calcOpt.getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_FALSE)	{
			setAEOnOffUnaryOpt(ABSTRACTEXPRTYPES.ABSTRACTEXPR_UOPT_FALSE, aexprChild, nNumOfOpts);
		} else if (calcOpt.getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_NOT)	{
			setAEOnOffUnaryOpt(ABSTRACTEXPRTYPES.ABSTRACTEXPR_UOPT_NOT, aexprChild, nNumOfOpts);
		} else if (calcOpt.getOperatorType() == BaseData.OPERATORTYPES.OPERATOR_TRANSPOSE)	{
	...
	public AEOnOffUnaryOpt(AbstractExpr aexprOrigin) throws ErrProcessor.JFCALCExpErrException, SMErrProcessor.JSmartMathErrException {
		copy(aexprOrigin);
	}
	...
	private void setAEOnOffUnaryOpt(ABSTRACTEXPRTYPES aeType, AbstractExpr aexprChild, int nNumOfOpts) throws SMErrProcessor.JSmartMathErrException {
		menumAEType = aeType;
	...
	public BaseData.DataClass evaluateAExprQuick(
        ...
        if (menumAEType == ABSTRACTEXPRTYPES.ABSTRACTEXPR_UOPT_FALSE)	{
            datumValue.changeDataType(BaseData.DATATYPES.DATUM_BOOLEAN);
            if (mnNumofOpts % 2 == 1)	{
                datumValue.setDataValue((datumValue.getDataValue().isFalse())?  // datumValue has been converted to boolean, so we can use isFalse instead of is Actually false.
                        MFPNumeric.TRUE:MFPNumeric.FALSE, BaseData.DATATYPES.DATUM_BOOLEAN);
            }
        } else if (menumAEType == ABSTRACTEXPRTYPES.ABSTRACTEXPR_UOPT_NOT)	{
            datumValue.changeDataType(BaseData.DATATYPES.DATUM_INTEGER);
            if (mnNumofOpts % 2 == 1)	{
                datumValue = ExprEvaluator.evaluateOneOperandCell(new BaseData.CalculateOperator(BaseData.OPERATORTYPES.OPERATOR_NOT, 1, true), datumValue);
            ...
            if (mnNumofOpts % 2 == 1)	{
                datumValue = ExprEvaluator.evaluateOneOperandCell(datumValue, new BaseData.CalculateOperator(BaseData.OPERATORTYPES.OPERATOR_TRANSPOSE, 1, false));
	...
	public AbstractExpr evaluateAExpr(
        ...
        if (aeChild instanceof AEConst) {
            BaseData.DataClass datumChild = ((AEConst)aeChild).getDataClassRef();
            BaseData.DataClass datumValue = new BaseData.DataClass();
            if (menumAEType == ABSTRACTEXPRTYPES.ABSTRACTEXPR_UOPT_FALSE)	{
                datumValue.copyTypeValueDeep(datumChild);
                datumValue.changeDataType(BaseData.DATATYPES.DATUM_BOOLEAN);
                ...
                if (mnNumofOpts % 2 == 1)	{
                    datumValue = ExprEvaluator.evaluateOneOperandCell(datumChild, new BaseData.CalculateOperator(BaseData.OPERATORTYPES.OPERATOR_TRANSPOSE, 1, false));
                } else if (mnNumofOpts != 0)	{	//even number of opts.
                    datumValue = ExprEvaluator.evaluateOneOperandCell(datumChild, new BaseData.CalculateOperator(BaseData.OPERATORTYPES.OPERATOR_TRANSPOSE, 1, false));
                    datumValue = ExprEvaluator.evaluateOneOperandCell(datumChild, new BaseData.CalculateOperator(BaseData.OPERATORTYPES.OPERATOR_TRANSPOSE, 1, false));
                }
	...
	public AbstractExpr simplifyAExpr(LinkedList<UnknownVarOperator.UnknownVariable> lUnknownVars,
		...
		if (aeCopy.maexprChild instanceof AEConst)	{
			BaseData.DataClass datumValue = ((AEConst)aeCopy.maexprChild).getDataClassCopy();
	...
	public boolean isNegligible() throws SMErrProcessor.JSmartMathErrException, ErrProcessor.JFCALCExpErrException {
		...
		if (menumAEType == ABSTRACTEXPRTYPES.ABSTRACTEXPR_UOPT_FALSE)	{
			if (mnNumofOpts % 2 == 0) 	{
				...
				BaseData.DataClass datum = ((AEConst)maexprChild).getDataClassCopy();
