...
public class ExprAnalyzer {
    ...
    public static AbstractExpr processTwoOperandCell(AbstractExpr aeFirstOperand,
                                                     BaseData.CalculateOperator COPTROperator, AbstractExpr aeSecondOperand) throws ErrProcessor.JFCALCExpErrException, SMErrProcessor.JSmartMathErrException {
        ...
        case OPERATOR_SUBTRACT:
            listChildren.add(aeFirstOperand);
            listChildren.add(aeSecondOperand);
            listOpts.add(new BaseData.CalculateOperator(BaseData.OPERATORTYPES.OPERATOR_ADD, 2));
            listOpts.add(new BaseData.CalculateOperator(COPTROperator.getOperatorType(), 2));
    ...
    public static AbstractExpr processOneOperandCell(AbstractExpr aeOperand,
            BaseData.CalculateOperator COPTROperator) throws ErrProcessor.JFCALCExpErrException, SMErrProcessor.JSmartMathErrException {
        AbstractExpr aeReturn = AEInvalid.AEINVALID;
        
        switch (COPTROperator.getOperatorType()) {
        case OPERATOR_TRANSPOSE:
            aeReturn = new AEOnOffUnaryOpt(AbstractExpr.ABSTRACTEXPRTYPES.ABSTRACTEXPR_UOPT_TRANSPOSE, aeOperand, 1);
            break;
        case OPERATOR_FACTORIAL:
            aeReturn = new AEUnaryOpt(AbstractExpr.ABSTRACTEXPRTYPES.ABSTRACTEXPR_UOPT_FACTORIAL, aeOperand);
            break;
        case OPERATOR_PERCENT:
            aeReturn = new AEUnaryOpt(AbstractExpr.ABSTRACTEXPRTYPES.ABSTRACTEXPR_UOPT_PERCENT, aeOperand);
            break;
        default:
    ...
    public static AbstractExpr processOneOperandCell(BaseData.CalculateOperator COPTROperator,
            AbstractExpr aeOperand) throws ErrProcessor.JFCALCExpErrException, SMErrProcessor.JSmartMathErrException {
        AbstractExpr aeReturn = AEInvalid.AEINVALID;
        switch (COPTROperator.getOperatorType()) {
        case OPERATOR_FALSE:
            aeReturn = new AEOnOffUnaryOpt(AbstractExpr.ABSTRACTEXPRTYPES.ABSTRACTEXPR_UOPT_FALSE, aeOperand, 1);
            break;
        case OPERATOR_NOT:
            aeReturn = new AEOnOffUnaryOpt(AbstractExpr.ABSTRACTEXPRTYPES.ABSTRACTEXPR_UOPT_NOT, aeOperand, 1);
            break;
        ...
        case OPERATOR_NEGSIGN:
            ...
            listOpts.add(new BaseData.CalculateOperator(COPTROperator.getOperatorType(), 1, true));
            aeReturn = new AEPosNegOpt(listChildren, listOpts);
            break;
        default:
            throw new ErrProcessor.JFCALCExpErrException(ErrProcessor.ERRORTYPES.ERROR_INCORRECT_MONADIC_OPERATOR);
    ...
    public static AbstractExpr processIndex(AbstractExpr aeToBeIndex, AbstractExpr aeIndex) throws ErrProcessor.JFCALCExpErrException, SMErrProcessor.JSmartMathErrException {
        return new AEIndex(aeToBeIndex, aeIndex);
    }
    
    public static AbstractExpr analyseExpression(String strExpression, BaseData.CurPos curpos)
            throws ErrProcessor.JFCALCExpErrException, SMErrProcessor.JSmartMathErrException, InterruptedException    {
        return analyseExpression(strExpression, curpos, new LinkedList<VariableOperator.Variable>());
    }
    
    public static AbstractExpr analyseExpression(String strExpression,
                                                 BaseData.CurPos curpos, LinkedList<VariableOperator.Variable> listPseudoConsts)
            throws ErrProcessor.JFCALCExpErrException, SMErrProcessor.JSmartMathErrException, InterruptedException    {
        LinkedList<AbstractExpr> tsAbstractExpr = new LinkedList<AbstractExpr>();
        LinkedList<BaseData.CalculateOperator> tsCalculateOperators = new LinkedList<BaseData.CalculateOperator>();

        BaseData.BoundOperator BOPTRCurboptr;
        BaseData.CalculateOperator COPTRStackTopcoptr;
        BaseData.CalculateOperator COPTRCurcoptr = new BaseData.CalculateOperator();
        ...
        while (curpos.m_nPos < strExpression.length()) {
            ...
            if (ElemAnalyzer.isBoundOperatorChar(strExpression, curpos.m_nPos)) {
                ...
                // in GetBoundOperator
                if (BOPTRCurboptr.isEqual(BaseData.OPERATORTYPES.OPERATOR_LEFTPARENTHESE)) {
                ...
                else if (BOPTRCurboptr.isEqual(BaseData.OPERATORTYPES.OPERATOR_RIGHTPARENTHESE)) {
                    if (nRecursionStartPosition == 0) {
                        throw new ErrProcessor.JFCALCExpErrException(ErrProcessor.ERRORTYPES.ERROR_UNMATCHED_RIGHTPARENTHESE);
                    }
                    ...
                    if (nLastPushedCellType != 1) {
                        throw new ErrProcessor.JFCALCExpErrException(
                                ErrProcessor.ERRORTYPES.ERROR_LACK_OPERAND);
                    }
                    ...
                    while (COPTRStackTopcoptr != null) {
                        if (COPTRStackTopcoptr.getOperandNum() == 1) {
                            if (aeCurExpr == null) {
                                throw new ErrProcessor.JFCALCExpErrException(ErrProcessor.ERRORTYPES.ERROR_LACK_OPERAND);
                            }
            ...
            /* A "," */
                ...
                throw new ErrProcessor.JFCALCExpErrException(ErrProcessor.ERRORTYPES.ERROR_CAN_NOT_IDENTIFIED_CHARACTER);
            }
