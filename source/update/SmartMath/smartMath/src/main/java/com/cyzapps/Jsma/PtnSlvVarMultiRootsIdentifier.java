...
public class PtnSlvVarMultiRootsIdentifier {
	...
	public PtnSlvVarMultiRootsIdentifier(PatternManager.ABSTRACTEXPRPATTERNS enumAEPType,
					PatternSetByString pss,
					String strVariable,
					String strSolveVarExpr,
					String strNumOfRoots)
					throws JSmartMathErrException, ErrProcessor.JFCALCExpErrException, InterruptedException	{
		setPtnSlvVarMultiRootsIdentifier(enumAEPType, pss, strVariable, strSolveVarExpr, strNumOfRoots);
	}
	
	public void setPtnSlvVarMultiRootsIdentifier(PatternManager.ABSTRACTEXPRPATTERNS enumAEPType,
					PatternSetByString pss,
					String strVariable,
					String strSolveVarExpr,
					String strNumOfRoots)
        ...
        for (int idx1 = 0; idx1 < mpss.mstrarrayPseudoConsts.length; idx1 ++)	{
            AbstractExpr aexprTmp = ExprAnalyzer.analyseExpression(mpss.mstrarrayPseudoConsts[idx1], new BaseData.CurPos());
            ...
            mpsa.maePConExprs[idx1] = ExprAnalyzer.analyseExpression(mpss.mstrarrayPConExprs[idx1], new BaseData.CurPos());
        }
        
        AbstractExpr aexprTmp = ExprAnalyzer.analyseExpression(mstrVariable, new BaseData.CurPos());
        if (!(aexprTmp instanceof AEVar))	{
            throw new JSmartMathErrException(ERRORTYPES.ERROR_INVALID_ABSTRACTEXPRPATTERN);
        }
        maeVariable = (AEVar) aexprTmp;
        maeSolveVarExpr = ExprAnalyzer.analyseExpression(mstrSolveVarExpr, new BaseData.CurPos());
		mpsa.maePattern = ExprAnalyzer.analyseExpression(mpss.mstrPattern, new BaseData.CurPos(), listPseudoConsts);
	}
	...
	public static AbstractExpr calcExprValue(AbstractExpr aeOriginalExpr, AbstractExpr[] aeExprUnits, BaseData.DataClass[] datumValues) throws ErrProcessor.JFCALCExpErrException, JSmartMathErrException	{
		LinkedList<PatternManager.PatternExprUnitMap> listFromToMap = new LinkedList<PatternManager.PatternExprUnitMap>();
		for (int idx = 0; idx < aeExprUnits.length; idx ++)	{
			AEConst aeTo = new AEConst(datumValues[idx].cloneSelf());
			PatternManager.PatternExprUnitMap pi = new PatternManager.PatternExprUnitMap(aeExprUnits[idx], aeTo);
	 ...
	 */
	public static AbstractExpr replaceExprFunc4PCon(AbstractExpr aeToSolve, AbstractExpr aeOriginalExpr, LinkedList<PatternManager.PatternExprUnitMap> listPEUMap) throws ErrProcessor.JFCALCExpErrException, JSmartMathErrException, InterruptedException	{
		if (aeToSolve instanceof AEFunction
			...
			BaseData.DataClass[] datumValues = new BaseData.DataClass[((AEFunction)aeToSolve).mlistChildren.size()];
			...
			for (int idx = 0; idx < listChildren.size(); idx ++)	{
				...
				PatternManager.PatternExprUnitMap pi = new PatternManager.PatternExprUnitMap(listChildren.get(idx),
	...
	public static boolean canSingleExprMatchBackPattern(AbstractExpr aeOriginalExpr,
												...
												LinkedList<PatternManager.PatternExprUnitMap> listPEUMap,
	...
	public boolean isPatternMatch(AbstractExpr aeOriginalExpr,  // original expr hasn't been mostly simplifed coz need first call replace pattern then simplify most.
                                  ...
                                  LinkedList<PatternManager.PatternExprUnitMap> listPEUMap,    // the map of unknown variables to pattern units
                                  ...
                                  LinkedList<LinkedList<Variable>> lVarNameSpaces) throws ErrProcessor.JFCALCExpErrException, JSmartMathErrException, InterruptedException
	{
		...
		if (listOriginalExprVars.size() != listPatternVars.size())	{
			...
			for (int idx = 0; idx < listOriginalExprVars.size(); idx ++)	{
				listPEUMap.add(new PatternManager.PatternExprUnitMap(listOriginalExprVars.get(idx), listPatternVars.get(idx)));
			}
		}
        ...
        for (int idx2 = 0; idx2 < mpsa.maePConExprs.length; idx2 ++)	{
            ...
            if (!(aePConToSolve instanceof AEConst)) {  // some function cannot be simplified to AEConst data expr.
                aePConToSolve = new AEConst(new BaseData.DataClass(aePConToSolve));
            }
        ...
        for (int idx2 = 0; idx2 < mpss.mstrarrayPConRestricts.length; idx2 ++)	{
            BaseData.DataClass datumRestrictResult = exprEvaluator.evaluateExpression(mpss.mstrarrayPConRestricts[idx2], new BaseData.CurPos());
            if (datumRestrictResult.getDataType() == BaseData.DATATYPES.DATUM_ABSTRACT_EXPR) {
            ...
            if (datumRestrictResult.getDataType() != BaseData.DATATYPES.DATUM_ABSTRACT_EXPR) {
                datumRestrictResult.changeDataType(BaseData.DATATYPES.DATUM_BOOLEAN);
        ...
        for (int idx2 = 0; idx2 < mpss.mstrarrayPConConditions.length; idx2 ++)	{
            BaseData.DataClass datumConditionResult = exprEvaluator.evaluateExpression(mpss.mstrarrayPConConditions[idx2], new BaseData.CurPos());
            if (datumConditionResult.getDataType() == BaseData.DATATYPES.DATUM_ABSTRACT_EXPR) {
            ...
            if (datumConditionResult.getDataType() != BaseData.DATATYPES.DATUM_ABSTRACT_EXPR) {
                datumConditionResult.changeDataType(BaseData.DATATYPES.DATUM_BOOLEAN);
        ...
        mnRootNumber = -1;
        BaseData.DataClass datumNumOfRoots = exprEvaluator.evaluateExpression(mstrRootNumber, new BaseData.CurPos());
        if (datumNumOfRoots.getDataType() == BaseData.DATATYPES.DATUM_ABSTRACT_EXPR) {
        ...
        if (datumNumOfRoots.getDataType() != BaseData.DATATYPES.DATUM_ABSTRACT_EXPR) {
            datumNumOfRoots.changeDataType(BaseData.DATATYPES.DATUM_INTEGER);
        ...
        if (bcanSingleExprMatchBackPattern)	{
            ...
            for (int idx3 = 0; idx3 < listLocalPConVars.size(); idx3 ++)	{
                ...
                BaseData.DataClass datumValuePCon = listLocalPConVars.get(idx3).getSolvedValue();
                if (datumValuePCon.getDataType() == BaseData.DATATYPES.DATUM_ABSTRACT_EXPR) {
                    ...
                    if (aexprData instanceof AEConst && ((AEConst)aexprData).getDataClassRef().getDataType() != BaseData.DATATYPES.DATUM_ABSTRACT_EXPR) {
                ...
                if (datumValuePCon.isZeros(false))	{
                    BaseData.DataClass datumZero = new BaseData.DataClass();
	...
	public BaseData.DataClass[] solveOriginalExprUnit(PatternManager.PatternExprUnitMap peuMap, LinkedList<UnknownVariable> listPseudoConstVars)
        ...
        if (maeVariable.isEqual(aePatternUnit))	{
            ...
            BaseData.DataClass[] datumValueList;
            if (mnRootNumber > 1) {
                if (((AEConst)aeReturn).getDataClassRef().getDataType() == BaseData.DATATYPES.DATUM_REF_DATA)	{
                    datumValueList = ((AEConst)aeReturn).getDataClassRef().getDataList();   // because it is a matrix, so need not to call getDataClass
                } else if (((AEConst)aeReturn).getDataClassRef().getDataType() == BaseData.DATATYPES.DATUM_ABSTRACT_EXPR)	{    // for situation that data type is aexpr
                    datumValueList = new BaseData.DataClass[mnRootNumber];
                    for (int idx = 0; idx < mnRootNumber; idx ++) {
                        AbstractExpr aexpr = ((AEConst)aeReturn).getDataClassRef().getAExpr();
                        BaseData.DataClass datumIndexValue = new BaseData.DataClass(BaseData.DATATYPES.DATUM_INTEGER, new MFPNumeric(idx));
                        BaseData.DataClass[] arrayIdx = new BaseData.DataClass[1];
                        arrayIdx[0] = datumIndexValue;
                        BaseData.DataClass datumIdx = new BaseData.DataClass(arrayIdx);
                        AEIndex aeIndexValue = new AEIndex(aexpr, new AEConst(datumIdx));
                        datumValueList[idx] = new BaseData.DataClass(aeIndexValue);
                    }
                } else {
                    throw new JSmartMathErrException(ERRORTYPES.ERROR_INVALID_RESULT);
                }
            } else	{
                datumValueList = new BaseData.DataClass[1];
